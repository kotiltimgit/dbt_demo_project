{% materialization custom_merge_material, adapter='default' -%}
  ...
  {%- set  update_result = run_custom_query(
    ...
    {{ log('update_result' ~ "select COLUMN_NAME from " ~ database ~ ".INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA='" ~ schema ~ "' AND TABLE_NAME='" ~ table_name ~ "' AND COLUMN_NAME NOT IN " ~ update_exclude_columns_list ~ " ORDER BY ORDINAL_POSITION;", info=True) }}
